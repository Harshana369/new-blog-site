...
interface Props {
  className?: string
  post: TPostDetail
  headerStyle?: 'style1' | 'style2' | 'style3' | 'audio' | 'video' | 'gallery' | 'review'
}
...
const SingleHeaderContainer: FC<Props> = ({ className, post, headerStyle = 'style1' }) => {
  ...
  if (post.postType === 'review' && post.reviewData && post.product) {
    return (
      <>
        <div className="container">
          <Divider />
        </div>
        <header className={clsx('container mt-8 lg:mt-16', className)}>
          <div className="mx-auto max-w-4xl">
            <ReviewHeader product={post.product} reviewData={post.reviewData} />
            <div className="mt-8">
              <TitleAndMeta post={post} />
            </div>
          </div>
