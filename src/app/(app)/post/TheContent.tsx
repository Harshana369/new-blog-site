...
const portableTextComponents: PortableTextComponents = {
  types: {
    image: ({ value }) => {
      if (!value?.asset?._ref && !value?.src) return null
      return (
        <figure>
          <Image
            src={value.src || value.asset?._ref}
...
const TheContent = ({ content }: { content: unknown }) => {
  // If content is a Portable Text array (from Sanity), render with PortableText
  if (Array.isArray(content) && content.length > 0 && content[0]?._type) {
    ...
    <>
      ...
      <figure>
        <Image
          ...
          alt="Article illustration showing typography examples"
          sizes="(max-width: 1024px) 100vw, 1240px"
          className="rounded-2xl"
          width={1635}
          height={774}
