...
import ProductSpecTable from '@/components/Review/ProductSpecTable'
  ...
  return (
    <div className={`relative ${className}`}>
      <div className="single-content space-y-10">
        {/* REVIEW VERDICT - shown before content for review posts */}
        {post.postType === 'review' && post.reviewData && (
          <div className="mx-auto max-w-(--breakpoint-md) space-y-8">
            <ReviewVerdict reviewData={post.reviewData} />
            {post.reviewData.ratings?.length > 0 && (
              <RatingBreakdown ratings={post.reviewData.ratings} />
            )}
            {(post.reviewData.pros?.length > 0 || post.reviewData.cons?.length > 0) && (
              <ProsConsList pros={post.reviewData.pros || []} cons={post.reviewData.cons || []} />
            )}
          </div>
        )}
        ...
        {/* REVIEW SPECS + AFFILIATE LINKS - shown after content */}
        {post.postType === 'review' && post.product && (
          <div className="mx-auto max-w-(--breakpoint-md) space-y-8">
            {post.product.specifications && post.product.specifications.length > 0 && (
              <ProductSpecTable specifications={post.product.specifications} />
            )}
            {post.product.affiliateLinks && post.product.affiliateLinks.length > 0 && (
