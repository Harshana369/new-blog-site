...
const SafeSvgIcon = ({ svgString, label }: { svgString: string; label: string }) => {
  const cleanSvg = DOMPurify.sanitize(svgString, {
    USE_PROFILES: { svg: true, svgFilters: true },
    ADD_TAGS: ['svg', 'path', 'circle', 'rect', 'line', 'polyline', 'polygon', 'ellipse', 'g', 'defs', 'use'],
    ADD_ATTR: ['viewBox', 'width', 'height', 'fill', 'stroke', 'stroke-width', 'stroke-linecap', 'stroke-linejoin', 'stroke-miterlimit', 'd', 'cx', 'cy', 'r', 'rx', 'ry', 'x', 'y', 'x1', 'y1', 'x2', 'y2', 'points', 'transform', 'xmlns', 'color', 'class', 'strokeWidth', 'strokeLinecap', 'strokeLinejoin'],
  })
  return <div role="img" aria-label={label} dangerouslySetInnerHTML={{ __html: cleanSvg }} />
}
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      {currencies.map((item, index) => (
        ...
        >
          <SafeSvgIcon svgString={item.icon} label={`${item.name} currency icon`} />
